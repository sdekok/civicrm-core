 ...
 * Class CRM_Dedupe_BAO_QueryBuilder_CoupleGeneral
 */
class CRM_Dedupe_BAO_QueryBuilder_CoupleGeneral extends CRM_Dedupe_BAO_QueryBuilder {
  /**
   * @param $rg
   *
   * @return array
  ...
  public static function internal($rg) {
    $query = "
            ...
            WHERE contact1.contact_type = 'Couple'
              AND (contact1.spouse_first_name IS NULL OR contact2.spouse_first_name IS NULL OR contact1.spouse_first_name = contact2.spouse_first_name)
              AND (contact1.spouse_last_name IS NULL OR contact2.spouse_last_name IS NULL OR contact1.spouse_last_name = contact2.spouse_last_name)
              AND (contact1.spouse_middle_name IS NULL OR contact2.spouse_middle_name IS NULL OR contact1.spouse_middle_name = contact2.spouse_middle_name)
              AND (contact1.suffix_id IS NULL OR contact2.suffix_id IS NULL OR contact1.suffix_id = contact2.suffix_id)
              AND (contact1.spouse_suffix_id IS NULL OR contact2.spouse_suffix_id IS NULL OR contact1.spouse_suffix_id = contact2.spouse_suffix_id)
              ...
              AND (contact1.spouse_birth_date IS NULL OR contact2.spouse_birth_date IS NULL OR contact1.spouse_birth_date = contact2.spouse_birth_date)
