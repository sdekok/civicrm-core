 ...
 */
class CRM_Contact_Form_Edit_Couple {

  /**
   * This function provides the HTML form elements that are specific to the Couple Contact Type.
   *
  ...
  public static function buildQuickForm(&$form, $inlineEditMode = NULL) {
    ...
    if (!$inlineEditMode || $inlineEditMode == 1) {
      ...
      foreach ($nameFields as &$fix) {
        ...
        if ($fix === 'prefix' || $fix === 'suffix' || $fix ==='spouse_prefix' || $fix==='spouse_suffix') {
          // God, why god?
          $fix .= '_id';
        }
      }
      ...
      foreach ($nameFields as $name) {
        $props = array();
        if ($name === 'prefix_id' || $name === 'suffix_id' || $name ==='spouse_prefix_id' || $name === 'spouse_suffix_id') {
          ...
          else if($name == 'spouse_prefix_id')
          {
            $label = ts('Spouse Prefix');
          }
          else if($name == 'spouse_suffix_id')
          {
            $label = ts('Spouse Suffix');
          }
          //Update this with Spouse prefix and Spouse Suffice too
          $props = array('class' => 'eight', 'placeholder' => ' ', 'label' => $name == 'prefix_id' ? ts('Prefix') : $name =='spouse_prefix_id' ? ts('Prefix') :ts('Suffix'));
        }
    ...
    if (!$inlineEditMode || $inlineEditMode == 2) {
      ...
      $form->addField('spouse_nick_name');
  ...
  public static function formRule($fields, $files, $contactID = NULL) {
    ...
    CRM_Contact_Form_Contact::checkDuplicateContacts($fields, $errors, $contactID, 'Couple');
