 ...
 */
class CRM_Contact_BAO_Couple extends CRM_Contact_DAO_Contact {
  ...
  public static function format(&$params, &$contact) {
    ...
    foreach (array('first_name', 'middle_name', 'last_name', 'nick_name', 'formal_title', 'spouse_first_name', 'spouse_middle_name', 'spouse_last_name', 'spouse_nick_name', 'spouse_formal_title') as $displayField) {
    ...
    $spouseFirstName = CRM_Utils_Array::value('spouse_first_name', $params, '');
    $spouseMiddleName = CRM_Utils_Array::value('spouse_middle_name', $params, '');
    $spouseLastName = CRM_Utils_Array::value('spouse_last_name', $params, '');
    $spouseNickName = CRM_Utils_Array::value('spouse_nick_name', $params, '');
    $spouse_prefix_id = CRM_Utils_Array::value('spouse_prefix_id', $params, '');
    $spouse_suffix_id = CRM_Utils_Array::value('spouse_suffix_id', $params, '');
    $spouseFormalTitle = CRM_Utils_Array::value('spouse_formal_title', $params, '');

    // get prefix and suffix names
    $prefix = $suffix = NULL;
    $spouse_prefix = $spouse_suffix = NULL;
    ...
    if ($spouse_prefix_id) {
      $params['spouse_prefix'] = $spousePrefix = CRM_Core_PseudoConstant::getLabel('CRM_Contact_DAO_Contact', 'spouse_prefix_id', $spouse_prefix_id);
    }
    if ($spouse_suffix_id) {
      $params['spouse_suffix'] = $spouseSuffix = CRM_Core_PseudoConstant::getLabel('CRM_Contact_DAO_Contact', 'spouse_suffix_id', $spouse_suffix_id);
    }
    ...
    $couple = NULL;
    if ($contact->id) {
      $couple = new CRM_Contact_BAO_Contact();
      $couple->id = $contact->id;
      if ($couple->find(TRUE)) {
        ...
        foreach (array('last', 'middle', 'first', 'nick', 'spouse_last', 'spouse_middle', 'spouse_first', 'spouse_nick') as $name) {
          $dbName = "{$name}_name";
          $value = $couple->$dbName;
        ...
        foreach (array('prefix', 'suffix', 'spouse_prefix', 'spouse_suffix') as $name) {
          $dbName = "{$name}_id";
          $value = $couple->$dbName;
          if ($value && !empty($params['preserveDBName'])) {
            $useDBNames[] = $name;
        ...
        if ($couple->formal_title && !empty($params['preserveDBName'])) {
        ...
        foreach (array('Last', 'Middle', 'First', 'Nick') as $name) {
          $phpName = "spouse{$name}Name";
          
          $lowerCaseName = strtolower($name);
          $dbName = "spouse_{$lowerCase}_name";
          $value = $couple->$dbName;
          if (in_array($name, $useDBNames)) {
            $params[$dbName] = $value;
            $contact->$dbName = $value;
        ...
        if (in_array('formal_title', $useDBNames)) {
          $params['formal_title'] = $couple->formal_title;
          $contact->formal_title = $couple->formal_title;
          $formalTitle = $couple->formal_title;
        }
        elseif (array_key_exists('formal_title', $params)) {
          $formalTitle = $params['formal_title'];
        }
        elseif ($couple->formal_title) {
          $formalTitle = $couple->formal_title;
        }
        
        if (in_array('spouse_formal_title', $useDBNames)) {
          $params['spouse_formal_title'] = $couple->spouse_formal_title;
          $contact->spouse_formal_title = $couple->spouse_formal_title;
          $spouseFormalTitle = $couple->formal_title;
        }
        elseif (array_key_exists('spouse_formal_title', $params)) {
          $spouseFormalTitle = $params['spouse_formal_title'];
        }
        elseif ($couple->formal_title) {
          $formalTitle = $couple->formal_title;
        }
    ...
    foreach (array('lastName', 'firstName', 'middleName', 'spouseLastName', 'spouseFirstName', 'spouseMiddleName') as $fld) {
      $$fld = trim($$fld);
    }

    if ($lastName || $firstName || $middleName || $spouseFirstName) {
      ...
      $nameParams = array(
        ...
        'spouse_nick_name' => $spouseNickName,
        'spouse_suffix' => $spouseSuffix,
        'spouse_prefix' => $spousePrefix,
        'spouse_prefix_id' => $spouse_prefix_id,
        'spouse_suffix_id' => $spouse_suffix_id,
        'spouse_formal_title' => $spouseFormalTitle,
      );
      ...
      //build the sort name.
      $format = Civi::settings()->get('couple_sort_name_format');
      ...
      if(!empty($params['spouse_first_name']) && !empty($params['spouse_last_name'])){
        if($params['spouse_last_name'] === $params['last_name'])
        {
          $format = Civi::settings()->get('couple_display_name_format');
        }
        else
        {
          $format = Civi::settings()->get('couple_display_name_multi_last_name_format');
        }   
      }
      else if(!empty($params['spouse_first_name']))
      {
        $format = Civi::settings()->get('couple_display_name_format');
      }
      else
  ...
  public static function updateDisplayNames(&$ids, $action) {
    ...
    foreach ($ids as $key => $value) {
      switch ($key) {
        ...
        case 'individualSuffix':
          ...
          break 2;
        case 'spousePrefix':
          $fieldName = 'spouse_prefix_id';
          $fieldValue = $value;
          break 2;
        case 'spouseSuffix':
          $fieldName = 'spouse_suffix_id';
          $fieldValue = $value;
          break 2;
      }
    }
    ...
    // query for the affected couples
  ...
  public function displayName() {
    ...
    $spouse_prefix = CRM_Core_PseudoConstant::get('CRM_Contact_DAO_Contact', 'spouse_prefix_id');
    $spouse_suffix = CRM_Core_PseudoConstant::get('CRM_Contact_DAO_Contact', 'spouse_suffix_id');
    
    if($this->$spouse_firstName)
    {
      $lastNameToUse = ($this->spouse_last_name ? $this->spouse_last_name : $this->last_name);
      return str_replace('  ', ' ', trim($prefix[$this->prefix_id] . ' and ' . $spouse_prefix[$this->$spouse_prefix_id] . ' ' . $this->first_name . ' ' . $this->middle_name . ' ' . $this->last_name . ' ' . $suffix[$this->suffix_id] . ' and ' . $this->spouse_first_name . ' ' . $this->spouse_middle_name . ' ' . $lastNameTouse . ' ' . $spouse_suffix[$this->spouse_suffix_id]));
    }
